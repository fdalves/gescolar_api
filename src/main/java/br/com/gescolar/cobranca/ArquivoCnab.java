...
public abstract class ArquivoCnab implements ArquivoTemplate {
	...
	public Linha gerarLinha(TipoLinhaEnum tipoLinhaEnum) {
		Optional<Linha> linha = arquivolayout.getLinhas().stream().
	    filter(l -> l.getLinhaEnum().equals(tipoLinhaEnum)).
	    findFirst();
		if (linha.isPresent()) {
	...
	public void setValor(Linha linha, Integer posicao, String valor) {
		...
		if (Objects.isNull(valor)) throw new CobrancaException(String.format("Linha pos %s, Campo pos %s, Valor não pode ser nulo",linha.getPosicao(), posicao));  
	...
	private void validaCampo(Linha linha, Campo campo) {
		...
		if (valor.length() > length) 
			throw new CobrancaException(String.format("Linha pos %s, Campo pos %s, Valor tamanho incoreto. ", linha.getPosicao(), campo.getPosicao()));
